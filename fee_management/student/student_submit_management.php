...
$date = $nepali_date = $nepaliDate->full;
...
//=================== Extra fee collection KRISHNA MAGH 2075 =====================
  if(isset($_GET['add_extra_fee'])){
    ...
    $balance = $_POST['fee_amount'];
    $description=$_POST['description'];

    if (empty($selected_date)) {
      $selected_date = $nepali_date;
    }
    list($selectedDate_year, $selectedDate_month, $selectedDate_day) = explode('-', $selected_date);
    $selected_date = $selectedDate_year.'-'.(int)$selectedDate_month.'-'.(int)$selectedDate_day;
    ...
    if(empty($class_id)){
      ...
      foreach ($student_id_list as $key){
        ...
        $account->add_fee_into_student_due($std_id, $feetype_id, $balance, $description, $selected_date, $LOGIN_CAT, $LOGIN_ID, 1);
